...
async function createGuideline(req, res) {
  try {
    ...
    const [newGuideline, guidelineError] = await GuidelineService.createGuideline({ name, maxSocialPercentage, minMembers });
...
async function updateGuideline(req, res) {
  try {
    const [updatedGuideline, updateError] = await GuidelineService.updateGuideline(req.params.id, req.body);
